#!/usr/bin/env python3
"""Unified runner for DM Control Cheetah-Run experiments.

Consolidates all experiments for Cheetah-Run across methods:
  ...
  - ``sac``            : Unsupervised / Oracle SAC baseline (``train_SAC.py``)

Key Settings for Cheetah-Run:
  - Environment: ``cheetah_run``
  - Actor / Critic hidden dim: 1024, depth: 2, batch size: 1024
  - Actor / Critic lr: 0.0005
  - Mixture feedback budget: 4000, reward batch: 100, num_interact: 20000
  - Reward update: 50, reset update: 100
  - Experiment reward_lr defaults (per method configs):
    * pebble_mixture : 0.001 (SGD network lr), alpha_lr: 0.005
...
--------
  ...
  python scripts/cheetah_run/run_experiments.py --dry-run

  # Run pebble_mixture with [1,1,1,0] preset on specific seeds:
  python scripts/cheetah_run/run_experiments.py --method pebble_mixture --preset 1,1,1,0 --seeds 12345 23451

  # Run rime_mixture with [1,1,1,-1] preset:
  python scripts/cheetah_run/run_experiments.py --method rime_mixture --preset 1,1,1,-1

  # Run mixup_mixture with custom teacher betas:
  python scripts/cheetah_run/run_experiments.py --method mixup_mixture --teacher-betas 1 1 1 0

  # Run all mixture methods on [1,1,1,-1]:
  python scripts/cheetah_run/run_experiments.py --methods pebble_mixture rime_mixture mixup_mixture mixture_raw --preset 1,1,1,-1
"""
...
def build_cmd(
    ...
    """Build command list for running the selected cheetah_run experiment."""
...
def parse_args() -> argparse.Namespace:
    ...
    run_group.add_argument(
        ...
        type=str,
        default="cheetah_run",
        help="Environment name (default: cheetah_run)",
    )
    run_group.add_argument(
        "--dry-run",
        action="store_true",
    ...
    param_group.add_argument(
        ...
        help=(
            ...
            "(pebble_mixture: 0.001; rime_mixture, mixup_mixture, mixture_raw, pebble: 0.0003)"
        ),
    )
    param_group.add_argument(
        "--alpha-lr",
    ...
    param_group.add_argument(
        ...
        type=int,
        default=4000,
        help="Maximum feedback budget (default: 4000)",
    )
    param_group.add_argument(
        "--reward-batch",
        type=int,
        default=100,
        help="Reward query batch size (default: 100)",
    )
    param_group.add_argument(
        "--num-train-steps",
        type=int,
    ...
    param_group.add_argument(
        ...
        type=int,
        default=20000,
        help="Steps between teacher interactions (default: 20000)",
    )
    param_group.add_argument(
        "--reward-update",
        type=int,
        default=50,
        help="Reward model update epochs per query batch (default: 50)",
    )
    param_group.add_argument(
        "--reset-update",
        type=int,
        default=100,
        help="Reset update steps (default: 100)",
    )
    param_group.add_argument(
        "--feed-type",
        type=int,
...
def main() -> int:
    ...
    print("=" * 80)
    print("DM Control Cheetah-Run Experiment Runner")
    print("=" * 80)
    ...
    for method in methods:
        ...
        for betas in configs_to_run:
            ...
            for seed_idx, seed in enumerate(args.seeds):
                ...
                print("\n" + "#" * 88)
                print(
                    f"[{current_run}/{total_runs}] [cheetah_run] method={method} | β={beta_str} | seed={seed} | {lr_str}"
                )
                print("#" * 88)
