#!/usr/bin/env python3
"""Unified runner for Meta-World Door-Open experiments.

Consolidates all experiments for Door-Open across methods:
...
Key Settings for Door-Open:
  - Environment: ``metaworld_door-open-v2``
...
--------
  ...
  python scripts/door_open/run_experiments.py --dry-run

  # Run pebble_mixture with [1,1,1,0] preset on specific seeds:
  python scripts/door_open/run_experiments.py --method pebble_mixture --preset 1,1,1,0 --seeds 12345 23451

  # Run rime_mixture with [1,1,1,-1] preset:
  python scripts/door_open/run_experiments.py --method rime_mixture --preset 1,1,1,-1

  # Run mixup_mixture with custom teacher betas:
  python scripts/door_open/run_experiments.py --method mixup_mixture --teacher-betas 1 1 1 0

  # Run all mixture methods on [1,1,1,-1]:
  python scripts/door_open/run_experiments.py --methods pebble_mixture rime_mixture mixup_mixture mixture_raw --preset 1,1,1,-1
"""
...
# - pebble_mixture uses SGD (lr=0.05 on Door-Open)
...
def build_cmd(
    ...
    """Build command list for running the selected door_open experiment."""
...
def parse_args() -> argparse.Namespace:
    ...
    run_group.add_argument(
        "--env",
        type=str,
        default="metaworld_door-open-v2",
        help="Environment name (default: metaworld_door-open-v2)",
    )
    run_group.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> int:
    ...
    print("=" * 80)
    print("Meta-World Door-Open Experiment Runner")
    print("=" * 80)
    ...
    for method in methods:
        ...
        for betas in configs_to_run:
            ...
            for seed_idx, seed in enumerate(args.seeds):
                ...
                print("\n" + "#" * 88)
                print(
                    f"[{current_run}/{total_runs}] [door_open] method={method} | β={beta_str} | seed={seed} | {lr_str}"
                )
                print("#" * 88)
